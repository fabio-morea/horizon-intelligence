...
pp<-ggplot(df, aes(x = n_org, y = n_coll)) +
  geom_point(shape = 21, stroke = 1, aes(fill = color, size = weight)) +  
  scale_size_continuous(range = c(1, 12)) +  # size of bubbles
    ...
    xlim(0,900)+ ylim(0,9000)+
    geom_text(data = subset(df, year %in% c(2015, 2024)), 
            aes(label = year), vjust = 0.9, hjust = -0.4, size = 4, color = "black") +  # Add labels for specified years
  ...
  theme_minimal() + theme(aspect.ratio = .5)
pp
png(paste0('./figures/5-community time/networksyears.png'), height = 300, width = 800, units = "px", bg = "white")
pp
dev.off()
```
